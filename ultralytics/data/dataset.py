...
class YOLODataset(BaseDataset):
    ...
    def get_labels(self):
        ...
        cache_path = Path(self.label_files[0]).parent.with_suffix(".cache") if self.hyp.cache_path is None else self.hyp.cache_path
        try:
    ...
    def build_transforms(self, hyp=None):
        ...
        if self.augment:
            ...
            transforms = Compose([LetterBox(new_shape=(self.imgsz, self.imgsz), scaleup=False, scale_fill=True)])
        transforms.append(
            Format(
                bbox_format="xywh",
                normalize=True,
    ...
    def collate_fn(batch):
        ...
        for i, k in enumerate(keys):
            ...
            if k in {"masks", "keypoints", "bboxes", "cls", "segments", "obb"}:
                value = torch.cat(value, 0)
            if k in ("im_file","ori_shape","ratio_pad") and isinstance(value[0], (list, tuple)):
